...
last_value(times) over (partition by grp order by grp, times rows between unbounded preceding and unbounded following) last_log_on
from (
...
where status = 'on') y)
, cte_final as(
select *, lead(times) over (order by times) as log_off
from login_details l
left join cte c on l.times = c.last_log_on)
select log_on ,log_off, extract (min from(log_off - log_on)) as duration 
from cte_final
